...
export async function reviewCard(userId, cardId, correct, table = 'cards') {
  ...
  if (correct) {
    ...
    if (score < 50) {
      ...
      const threeInRowBonus = newStreak >= 3 && newStreak <= 4 ? (80 + baseIncrease) : 0;
      const fiveInRowBonus = newStreak >= 5 && newStreak <= 9 ? (135 + (baseIncrease * 6)) : 0;
      const tenInRowBonus = newStreak >= 10  && newStreak <= 14 ? (280 + (baseIncrease * 12)) : 0;
      const fifteenInRowBounus = newStreak >= 15 ? (410 + (baseIncrease * 30)) : 0;
